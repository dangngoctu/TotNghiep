...
var getReport = function(){
    $.ajax({
        url: base_admin + "/home/ajax/ajax_report_user?tag=" + type + "&month=" + period_performance,
        type: "get",
        ...
        $.each(list_report, function(key, value) {
            if(type == 'early') {
                $('#bread-type, #title-page, #title-report').html('Leave Early');
                dataAudit.push(value.time);
            } else if (type == 'late') {
                $('#bread-type, #title-page, #title-report').html('Come Late');
                dataAudit.push(value.time);
            }
        ...
        if(typeof auditChart != "undefined") {
            ...
            if(type == 'Early') {
                auditChart.data.datasets[0].backgroundColor = window.chartColors.green;
                auditChart.data.datasets[0].label = 'Times leave early';
            } else {
                auditChart.data.datasets[0].label = 'Times come late';
            }
            auditChart.update();
        }
    });
